...
def rank_supplier_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "has_visible_price" in out.columns:
        visible_mask = _as_bool_series(out, "has_visible_price")
        cas_mask = _as_bool_series(out, "cas_exact_match") if "cas_exact_match" in out.columns else pd.Series(False, index=out.index)
        out.loc[visible_mask & cas_mask, "score"] += 15
        out.loc[visible_mask & ~cas_mask, "score"] += 5
        out.loc[visible_mask & ~cas_mask, "score"] -= 18
    if "price_pairing_confidence" in out.columns:
        ...
        out.loc[out["price_pairing_confidence"].astype(str).eq("LOW"), "score"] += 3

...
def _trust_decision(row: pd.Series) -> str:
    ...
    cas_confirmed = bool(row.get("cas_exact_match", False))
    if page_type == "failed_source":
        return "Manual review: extraction failed"
    if has_price and str(row.get("page_type")) == "search_page":
        return "Reject price lead: search page, product page not resolved"
    if has_price and not cas_confirmed:
        return "Candidate product-page price lead: CAS not confirmed; verify manually"
    if page_type == "search_page" and not has_price:
    ...
    if has_price and pairing in {"HIGH", "MEDIUM"} and cas_confirmed:
        return "Verified catalog price: use for comparison and curve model"
    if has_price:
        return "Show price lead, but verify pairing and CAS manually"
    return "Supplier lead / RFQ candidate"

def _trust_warning(row: pd.Series) -> str:
    warnings: list[str] = []
    has_price = pd.notna(row.get("listed_price_usd"))
    if has_price and not bool(row.get("cas_exact_match", False)):
        warnings.append("public price found but requested CAS was not confirmed on the product identity block")
    if bool(row.get("price_noise_flag")):
...
def enrich_procurement_trust(df: pd.DataFrame, required_purity: str | None = None) -> pd.DataFrame:
    ...
    out["verified_public_price"] = has_price & has_cas & pairing_ok & page_ok & ~noise_mask
    out["tentative_price"] = has_price & ~out["verified_public_price"] & ~noise_mask
    out["lead_cas_match"] = out["cas_exact_match"].map(lambda x: "confirmed" if bool(x) else "unconfirmed")
    out["price_validation_level"] = "NONE"
    out.loc[out["verified_public_price"], "price_validation_level"] = "VERIFIED"
    out.loc[out["tentative_price"], "price_validation_level"] = "CANDIDATE_REVIEW"
    out.loc[noise_mask, "price_validation_level"] = "REJECTED_NOISE"
    # All non-noisy visible prices remain chartable as price leads, but only verified
    # mass-catalog rows below are eligible for the desired quantity curve model.
    search_page_mask = out["page_type"].eq("search_page")
    # v19 UI separation:
    # - lead_price_chart_eligible: every non-noisy public price lead, including lower-confidence candidates.
    # - catalog_chart_eligible: every non-noisy mass-based price row for charting, verified or candidate.
    # - bulk_estimate_eligible: only verified CAS/product rows enter the desired-quantity model.
    out["lead_price_chart_eligible"] = has_price & ~noise_mask & ~search_page_mask
    out["catalog_chart_eligible"] = has_price & ~noise_mask & form_ok
